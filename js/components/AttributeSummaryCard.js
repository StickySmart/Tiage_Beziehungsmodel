...
const AttributeSummaryCard = (function() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // RESONANZ-FILTER (R1-R4) für Alle Bedürfnisse Tabelle
    // ═══════════════════════════════════════════════════════════════════════════

    /**
     * Mapping: Resonanz-Dimensionen (R1-R4) → Taxonomie-Dimensionen (#D1-#D6)
     * Basierend auf ResonanzTreeView.RESONANZ_DIMENSIONEN
     */
    const RESONANZ_TO_TAXONOMIE = {
        'R1': ['#D3'],           // Leben → Nähe-Distanz
        'R2': ['#D1', '#D2'],    // Philosophie → Beziehungsphilosophie, Werte-Alignment
        'R3': ['#D4'],           // Dynamik → Autonomie
        'R4': ['#D5', '#D6']     // Identität → Kommunikation, Soziale-Kompatibilität
    };

    /**
     * Holt alle Kategorie-IDs für eine Resonanz-Dimension
     * @param {string} resonanzId - R1, R2, R3 oder R4
     * @returns {string[]} Array von Kategorie-IDs (#K1-#K18)
     */
    function getKategorienFuerResonanz(resonanzId) {
        const taxonomieDims = RESONANZ_TO_TAXONOMIE[resonanzId];
        if (!taxonomieDims) return [];

        const kategorien = [];
        const taxonomie = typeof TiageTaxonomie !== 'undefined' ? TiageTaxonomie : null;

        if (!taxonomie) {
            console.warn('[AttributeSummaryCard] TiageTaxonomie nicht verfügbar');
            return [];
        }

        taxonomie.init();

        // Sammle alle Kategorien für die Taxonomie-Dimensionen
        taxonomieDims.forEach(dimId => {
            const kats = taxonomie.getKategorienFuerDimension(dimId);
            kats.forEach(kat => {
                if (!kategorien.includes(kat.id)) {
                    kategorien.push(kat.id);
                }
            });
        });
    ...
    /**
     * Filtert die Alle-Bedürfnisse-Tabelle nach Suchbegriff
     * Unterstützt:
     * - Resonanz-Filter: R1, R2, R3, R4 (aktiviert alle zugehörigen Kategorien)
     * - Text-Suche: Sucht in Bedürfnis-ID und Label
     *
     * @param {string} query - Suchbegriff
     */
    function filterBySearch(query) {
        console.log('[AttributeSummaryCard] filterBySearch:', query);

        const normalizedQuery = query.trim().toUpperCase();

        // Prüfe ob Resonanz-Dimension gesucht wird (R1, R2, R3, R4)
        if (['R1', 'R2', 'R3', 'R4'].includes(normalizedQuery)) {
            filterByResonanz(normalizedQuery);
            return;
        }

        // Reset Filter wenn leer
        if (!query.trim()) {
            resetSearchFilter();
            return;
        }

        // Text-basierte Suche in Bedürfnis-Labels
        filterByText(query.trim().toLowerCase());
    }

    /**
     * Filtert nach Resonanz-Dimension (R1-R4)
     * Aktiviert alle Kategorien, die zu dieser Resonanz gehören
     *
     * @param {string} resonanzId - R1, R2, R3 oder R4
     */
    function filterByResonanz(resonanzId) {
        console.log('[AttributeSummaryCard] filterByResonanz:', resonanzId);

        // Hole alle Kategorien für diese Resonanz
        const kategorien = getKategorienFuerResonanz(resonanzId);
        console.log('[AttributeSummaryCard] Kategorien für', resonanzId + ':', kategorien);

        if (kategorien.length === 0) {
            console.warn('[AttributeSummaryCard] Keine Kategorien gefunden für', resonanzId);
            return;
        }

        // Setze DimensionKategorieFilter auf diese Kategorien
        if (typeof DimensionKategorieFilter !== 'undefined') {
            // Zuerst alle Filter zurücksetzen
            DimensionKategorieFilter.reset();

            // Dann alle Kategorien für diese Resonanz aktivieren
            kategorien.forEach(katId => {
                if (!DimensionKategorieFilter.isKategorieActive(katId)) {
                    DimensionKategorieFilter.toggleKategorie(katId);
                }
            });

            console.log('[AttributeSummaryCard] DimensionKategorieFilter gesetzt für', resonanzId);
        } else {
            // Fallback: Direkte CSS-Filterung wenn DimensionKategorieFilter nicht verfügbar
            applyDirectResonanzFilter(resonanzId, kategorien);
        }
    }

    /**
     * Filtert nach Text im Bedürfnis-Label oder ID
     * @param {string} query - Suchtext (lowercase)
     */
    function filterByText(query) {
        const needItems = document.querySelectorAll('.flat-need-item');
        let visibleCount = 0;

        needItems.forEach(item => {
            const needId = item.dataset.need || '';
            const label = item.querySelector('.flat-need-label')?.textContent?.toLowerCase() || '';

            // Suche in ID und Label
            const matches = needId.toLowerCase().includes(query) ||
                           label.includes(query);

            if (matches) {
                item.classList.remove('search-filter-hidden');
                visibleCount++;
            } else {
                item.classList.add('search-filter-hidden');
            }
        });

        console.log('[AttributeSummaryCard] Text-Suche:', query, '- Sichtbar:', visibleCount);

        // Update Subtitle mit Filterinfo
        updateFilterSubtitle(visibleCount, needItems.length, query);
    }

    /**
     * Direkter Resonanz-Filter (Fallback wenn DimensionKategorieFilter nicht verfügbar)
     * @param {string} resonanzId - R1-R4
     * @param {string[]} kategorien - Zugehörige Kategorien
     */
    function applyDirectResonanzFilter(resonanzId, kategorien) {
        const needItems = document.querySelectorAll('.flat-need-item');
        let visibleCount = 0;
        ...
        needItems.forEach(item => {
            const needId = item.dataset.need;
            if (!needId || !beduerfnisIds?.beduerfnisse?.[needId]) {
                item.classList.add('search-filter-hidden');
                return;
            }

            const needKat = beduerfnisIds.beduerfnisse[needId].kategorie;
            if (kategorien.includes(needKat)) {
                item.classList.remove('search-filter-hidden');
                visibleCount++;
            } else {
                item.classList.add('search-filter-hidden');
        ...
        console.log('[AttributeSummaryCard] Direkter Resonanz-Filter:', resonanzId, '- Sichtbar:', visibleCount);
        updateFilterSubtitle(visibleCount, needItems.length, resonanzId);
    }

    /**
     * Setzt den Such-Filter zurück
     */
    function resetSearchFilter() {
        // Entferne search-filter-hidden Klasse von allen Items
        const needItems = document.querySelectorAll('.flat-need-item.search-filter-hidden');
        needItems.forEach(item => {
            item.classList.remove('search-filter-hidden');
        });

        // Reset DimensionKategorieFilter wenn vorhanden
        if (typeof DimensionKategorieFilter !== 'undefined') {
            DimensionKategorieFilter.reset();
        }

        // Reset Subtitle
        const subtitle = document.querySelector('.flat-needs-subtitle');
        if (subtitle && currentFlatArchetypLabel) {
            const totalCount = document.querySelectorAll('.flat-need-item').length;
            subtitle.textContent = `Dein ${currentFlatArchetypLabel}-Profil (${totalCount} Bedürfnisse)`;
        }

        console.log('[AttributeSummaryCard] Such-Filter zurückgesetzt');
    }

    /**
     * Aktualisiert den Subtitle mit Filterinfo
     * @param {number} visibleCount - Anzahl sichtbarer Bedürfnisse
     * @param {number} totalCount - Gesamtanzahl
     * @param {string} filterText - Aktiver Filter
     */
    function updateFilterSubtitle(visibleCount, totalCount, filterText) {
        const subtitle = document.querySelector('.flat-needs-subtitle');
        if (subtitle && currentFlatArchetypLabel) {
            subtitle.textContent = `Dein ${currentFlatArchetypLabel}-Profil (${visibleCount} von ${totalCount} Bedürfnissen) - Filter: ${filterText}`;
        }
    }
    ...
    return {
        ...
        lockSelectedNeeds,
        // NEU: Such-Filter für Alle Bedürfnisse Tabelle (R1-R4 + Text)
        filterBySearch,
        resetSearchFilter
    };
})();
