 ...
 */
        ...
        // Expert mode password (simple hash - not for security, just to prevent accidental access)
        const EXPERT_MODE_HASH = 'tiage2024';
        ...
        // Initialize expert mode from localStorage
        function initExpertMode() {
            const stored = localStorage.getItem('tiageExpertMode');
            if (stored) {
                try {
                    const data = JSON.parse(stored);
                    expertModeActive = data.active || false;
                    Object.assign(expertFeatures, data.features || {});
                    applyExpertFeatures();
                } catch (e) {
                    console.log('Expert mode init failed:', e);
                }
            }
        }

        // Open expert mode modal
        function openExpertModeModal() {
            const modal = document.getElementById('expertModeModal');
            if (modal) {
                // Reset UI based on current state
                const loginForm = document.getElementById('expertModeLoginForm');
                const settings = document.getElementById('expertModeSettings');
                const passwordInput = document.getElementById('expertModePassword');
                const errorDiv = document.getElementById('expertModeError');

                if (expertModeActive) {
                    loginForm.style.display = 'none';
                    settings.style.display = 'block';
                    // Update checkboxes
                    document.getElementById('expertShowArchetypeNav').checked = expertFeatures.archetypeNav;
                } else {
                ...
                if (!expertModeActive && passwordInput) {
                    setTimeout(() => passwordInput.focus(), 100);
                }
            }
        }

        // Close expert mode modal
        function closeExpertModeModal(event) {
        ...
        function verifyExpertPassword() {
            ...
            if (password === EXPERT_MODE_HASH) {
                expertModeActive = true;
                saveExpertMode();

                // Switch to settings view
                document.getElementById('expertModeLoginForm').style.display = 'none';
                document.getElementById('expertModeSettings').style.display = 'block';
                document.getElementById('expertShowArchetypeNav').checked = expertFeatures.archetypeNav;
            } else {
                if (errorDiv) {
        ...
        // Toggle expert feature
        function toggleExpertFeature(feature, enabled) {
            expertFeatures[feature] = enabled;
            saveExpertMode();
            applyExpertFeatures();
        }

        // Apply expert features to UI
        function applyExpertFeatures() {
            // Archetype navigation in Synthese modal
            const archetypeNav = document.getElementById('pathosLogosArchetypeContext');
            if (archetypeNav) {
                archetypeNav.style.display = expertFeatures.archetypeNav ? 'flex' : 'none';
            }
        }

        // Save expert mode to localStorage
        function saveExpertMode() {
            localStorage.setItem('tiageExpertMode', JSON.stringify({
        ...
        function deactivateExpertMode() {
            expertModeActive = false;
            Object.keys(expertFeatures).forEach(key => expertFeatures[key] = false);
            saveExpertMode();
            applyExpertFeatures();
            closeExpertModeModal();
        }

        // Keyboard shortcut: Ctrl+Shift+E to open expert mode
        ...
        function updateSourceExplanation(archetypeKey, personData, dominanz, orientierung) {
            ...
            if (!factorsContainer) {
                console.warn('[SourceExplanation] Container not found');
                return;
            }
            ...
            // Get display values with robust fallbacks
            var archetypeName = archetypeKey ? archetypeKey.charAt(0).toUpperCase() + archetypeKey.slice(1) : 'Duo';

            // Not selected text (localized)
            var notSelectedText = 'Nicht gewählt';
            if (locale && locale.profileReview && locale.profileReview.sourceExplanation && locale.profileReview.sourceExplanation.notSelected) {
                notSelectedText = locale.profileReview.sourceExplanation.notSelected;
            }

            // Gender display
            var genderDisplay = notSelectedText;
            if (personData && personData.geschlecht) {
                var genderPrimary = personData.geschlecht.primary;
                var genderSecondary = personData.geschlecht.secondary;
                ...
                } else if (genderPrimary) {
                    genderDisplay = locale && locale.geschlecht && locale.geschlecht.primary && locale.geschlecht.primary[genderPrimary]
                        ? locale.geschlecht.primary[genderPrimary]
                        : genderPrimary;
                }
            }

            // Dominanz display with localization
            ...
            if (locale && locale.dominanz && locale.dominanz.types && locale.dominanz.types[dominanz]) {
                ...
                dominanzDisplay = dominanzDisplay.charAt(0).toUpperCase() + dominanzDisplay.slice(1);
            }

            // Orientierung display with localization
            ...
            if (locale && locale.orientierung && locale.orientierung.types && locale.orientierung.types[orientierung]) {
                ...
                // Capitalize first letter as fallback
                orientierungDisplay = orientierungDisplay.charAt(0).toUpperCase() + orientierungDisplay.slice(1);
            }

            // Build HTML with all factors
            var html = '';
            ...
            console.log('[SourceExplanation] Updated with:', archetypeName, genderDisplay, dominanzDisplay, orientierungDisplay);
        }
