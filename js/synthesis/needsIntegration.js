...
TiageSynthesis.NeedsIntegration = {
     ...
     * basierend auf der Ähnlichkeit zwischen zwei Personen (DUO-MODUS).
     *
     * Im Duo-Modus werden die Bedürfnisse von ICH mit denen von PARTNER verglichen.
     * Bei identischen Werten (z.B. beide 100) ergibt sich R = 1.5 (Maximum).
     *
     * @param {object} needs1 - Bedürfnisse von Person 1 (ICH) { needKey: value }
     * @param {object} needs2 - Bedürfnisse von Person 2 (PARTNER) { needKey: value }
     * @returns {object} { R1, R2, R3, R4, leben, philosophie, dynamik, identitaet, enabled }
     */
    calculateDuoResonance: function(needs1, needs2) {
        var constants = TiageSynthesis.Constants;

        if (!needs1 || !needs2) {
            console.warn('[NeedsIntegration.calculateDuoResonance] Fehlende Needs-Daten');
            return {
        ...
        // Hole relevante Bedürfnisse pro Dimension
        var needsGroups = {
        ...
        var result = {
            leben: this._calculateDuoSingleResonance(needs1, needs2, needsGroups.leben),
            philosophie: this._calculateDuoSingleResonance(needs1, needs2, needsGroups.philosophie),
            dynamik: this._calculateDuoSingleResonance(needs1, needs2, needsGroups.dynamik),
            identitaet: this._calculateDuoSingleResonance(needs1, needs2, needsGroups.identitaet),
            enabled: true
        };
        ...
        console.log('[NeedsIntegration.calculateDuoResonance] Ergebnis:', {
            R1_leben: result.R1,
    ...
    /**
     * Berechnet R für eine einzelne Dimension im Duo-Modus
     * Vergleicht die Bedürfniswerte beider Personen direkt.
     *
     * Formel: R = 0.5 + (Ähnlichkeit / 100)
     * Ähnlichkeit = 100 - durchschnittliche Abweichung
     *
     * @private
     */
    _calculateDuoSingleResonance: function(needs1, needs2, needsList) {
        if (!needsList || needsList.length === 0) {
            return 1.0; // Neutral
        }
        ...
        for (var i = 0; i < needsList.length; i++) {
            ...
            var val1 = this._getNeedValue(needs1, null, needKey);
            var val2 = this._getNeedValue(needs2, null, needKey);
     ...
     *
     * HINWEIS: Diese Funktion vergleicht EINE Person gegen archetyp-typische Werte.
     * Für Duo-Modus (ICH vs PARTNER) verwende calculateDuoResonance() stattdessen.
     *
