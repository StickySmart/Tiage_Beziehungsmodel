# Forschungsquellen für Psychologische Profile

Dieses Dokument listet alle wissenschaftlichen Quellen auf, die zur Erstellung der 216 psychologischen Profile verwendet wurden.

## 1. Big Five Persönlichkeitsmodell

### Grundlagenforschung
- **McCrae & Costa (1997)**: Five-Factor Model of Personality - das dominante Persönlichkeitsmodell
- [Big Five Personality Traits - Simply Psychology](https://www.simplypsychology.org/big-five-personality.html)
- [Big Five Personality Traits - Wikipedia](https://en.wikipedia.org/wiki/Big_Five_personality_traits)
- [Big Five Personality Traits - EBSCO Research](https://www.ebsco.com/research-starters/social-sciences-and-humanities/big-five-personality-traits)

### Aktuelle Studien (2020-2024)
- **Trajectories of Big Five Personality Traits (PMC, 2020)**: Koordinierte Analyse von 16 Längsschnittstudien mit über 60.000 Teilnehmern
- **Hamdani et al. (2024)**: Big Five und soziale Unterstützung während COVID-19 (N=3.330)
- **Rauthmann (2024)**: "Personality is much more than self-reported Big Five traits"

### Kernbefunde
- Neurotizismus ist der stärkste Prädiktor für psychische Gesundheitsprobleme
- Extraversion und Offenheit korrelieren mit authentischen Führungsverhaltensweisen
- 50% Heritabilität der Big Five Traits

---

## 2. Dominanz & BDSM-Forschung

### Schlüsselstudien
- **SMSNA (2023)**: [Personality Type Associated With BDSM Role](https://www.smsna.org/patients/news/personality-type-is-associated-with-bdsm-role-says-recent-study)
- **ISSM Study**: [Personality and BDSM Roles](https://www.issm.info/sexual-health-headlines/study-explores-personality-and-bdsm-roles)
- **Wismeijer & van Assen (2013)**: BDSM practitioners psychological characteristics
- [Psychology Today: Sexual Science of Masochism](https://www.psychologytoday.com/us/blog/myths-desire/202006/the-sexual-science-masochism-and-submission)

### Kernbefunde

#### Dominante Personen:
- Höhere PAI-DOM Scores (Personality Assessment Inventory Dominance)
- Tendenz zu höherer Extraversion
- Niedrigere Agreeableness
- Sensation-seeking Verhalten

#### Submissive Personen:
- Niedrigere PAI-DOM Scores
- Höhere Agreeableness
- Konflikt-vermeidend
- Niedrigere Assertivität

#### Switch-Personen:
- Hohe Flexibilität
- Niedriger Neurotizismus
- Sehr hohe Openness to Experience
- 37% der BDSM-Praktizierenden identifizieren sich als Switch

### Geschlechterverteilung (Netherlands Study)
- 75.6% der SM-praktizierenden Frauen bevorzugen submissive Rolle
- Nur 33.4% der SM-praktizierenden Männer bevorzugen submissive Rolle

---

## 3. Sexuelle Orientierung & Persönlichkeit

### Meta-Analysen
- **Allen et al. (2020)**: [Personality and Sexual Orientation Meta-analysis](https://pubmed.ncbi.nlm.nih.gov/32510233/) - 21 Studien, 377.951 Teilnehmer
- [ScienceDirect: Sexual Orientation and Openness](https://www.sciencedirect.com/science/article/abs/pii/S0191886918302290)
- [Psychology Today: Personality and Sexual Orientation](https://www.psychologytoday.com/us/blog/finding-new-home/202011/the-relationship-between-personality-and-sexual-orientation)

### Kernbefunde

#### Heterosexuelle:
- Niedrigste Openness to Experience aller Orientierungen
- Höchste Conformität zu Geschlechterstereotypen

#### Homosexuelle:
- Höhere Openness als Heterosexuelle
- Gay men: Höherer Neurotizismus als heterosexuelle Männer
- Tendenziell weniger traditionell

#### Bisexuelle:
- **HÖCHSTE Openness to Experience** aller Orientierungen (Curvilinear Pattern)
- Niedrigere Conscientiousness
- Höheres Sensation-seeking
- Größte Flexibilität in Identität und Beziehungen

> "For openness, rather than a straightforward difference between heterosexual and homosexual persons, results indicate a curvilinear association in which bisexual individuals score highest."

---

## 4. Polyamorie & Konsensuelle Nicht-Monogamie

### Schlüsselstudien
- **Moors et al. (2021)**: [Polyamory National Sample Study](https://pmc.ncbi.nlm.nih.gov/articles/PMC8023325/)
- **2024 Scoping Review**: [Research on Polyamory and CNM](https://onlinelibrary.wiley.com/doi/full/10.1111/jftr.12546) - 78 Journals, 108 Psychologie-Abteilungen
- [Defining Polyamory: Thematic Analysis](https://pmc.ncbi.nlm.nih.gov/articles/PMC8321986/)

### Kernbefunde
- Polyamore zeigen **ähnliche Beziehungsqualität** wie Monogame (Vertrauen, Commitment, Liebe)
- Oft **niedrigere Eifersucht** und **höhere sexuelle Zufriedenheit**
- Hohe Kommunikationsanforderungen
- Größere persönliche Wachstumsmöglichkeiten

### Historischer Kontext
- 1975: 33% der Therapeuten hypothetisierten Persönlichkeitsstörungen bei offenen Beziehungen
- Moderne Forschung: Keine pathologischen Unterschiede zu Monogamen

---

## 5. Single-Lebensstil & Autonomie

### Schlüsselstudien
- **Stern et al. (2024)**: [Differences Between Lifelong Singles and Ever-Partnered Individuals](https://journals.sagepub.com/doi/10.1177/09567976241286865) - 27 Länder
- **DePaulo (2017)**: Singled Out - Singles-Studies Perspective
- [Frontiers Psychology (2025): Singlehood Latent Profile Analysis](https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2025.1509349/full)
- **Self-Determination Theory** (Ryan & Deci): Autonomie als Grundbedürfnis

### Kernbefunde
- Singles berichten **höchste Autonomie-Level**
- Singles sind weniger extravertiert und gewissenhaft als Partnered
- Zwei glücklichste Single-Typen:
  1. Romantische Bindung vermeidend (oft Frauen)
  2. Casual Dating (oft Männer)
- Autonomie = Schlüssel zum Wohlbefinden bei Singles

> "The happiest singles fall into two types: those avoiding romantic attachment and those casually dating."

---

## 6. Geschlechterunterschiede in Kommunikation

### Grundlagenwerk
- **Tannen, Deborah (1990)**: You Just Don't Understand - Gender Differences in Communication

### Weitere Quellen
- [Ohio State: Gender Issues in Interpersonal Communication](https://ohioline.osu.edu/factsheet/FLM-FS-4-02-R10)
- [UT Permian Basin: How Men and Women Communicate Differently](https://online.utpb.edu/about-us/articles/communication/how-men-and-women-communicate-differently/)
- [UCLA: Gendered Communication Patterns in Conflict](https://languagedlife.ucla.edu/sociolinguistics/the-language-of-love-gendered-communication-patterns-in-conflict/)

### Kernbefunde

#### Männer:
- Sprache für Status/Dominanz
- Sachlich-direkter Stil
- Fokus auf Problemlösung
- Weniger emotionaler Ausdruck

#### Frauen:
- Sprache für Verbindung
- Empathisch-verbindender Stil
- Fokus auf Beziehungserhalt
- Höhere emotionale Expressivität

> **Wichtig**: Unterschiede innerhalb der Geschlechter sind größer als zwischen ihnen.

---

## 7. Non-binäre Identität & Persönlichkeit

### Schlüsselstudien
- [Tandfonline: Psychological Functioning in Non-binary Adults](https://www.tandfonline.com/doi/full/10.1080/0092623X.2021.1950087)
- [ScienceDirect: Non-binary Gender/Sex Identities (2022)](https://www.sciencedirect.com/science/article/pii/S2352250X22002202)
- [APA Division 44: Nonbinary Fact Sheet](https://www.apadivisions.org/division-44/resources/nonbinary-fact-sheet.pdf)

### Big Five bei Non-binären Jugendlichen
| Trait | Durchschnitt (1-5 Skala) |
|-------|-------------------------|
| Openness | 3.83 (höchster) |
| Neuroticism | 3.60 (erhöht) |
| Agreeableness | 3.52 |
| Conscientiousness | 2.94 |
| Extraversion | 2.80 |

### Kernbefunde
- ~35% der Trans-Erwachsenen in USA identifizieren sich als Non-binär
- ~70% der Non-binären wurden bei Geburt als weiblich zugewiesen
- Höhere psychische Belastung als binär-identifizierende Personen
- Sehr hohe Openness to Experience

---

## 8. Philosophische Grundlagen

### Robert Pirsig - Metaphysics of Quality

**Werk**: Lila: An Inquiry into Morals (1991)

**Kernkonzepte**:
- **Static Quality**: Feste Muster, Stabilität, Vorhersagbarkeit
- **Dynamic Quality**: Bewegung, Veränderung, Evolution
- Balance zwischen beiden als Ideal

**Anwendung auf Profile**:
- Single/Polyamor = Hohe dynamische Qualität
- Duo = Hohe statische Qualität
- Switch/Ausgeglichen = Balance

---

### Tiages Widerspruch zu Pirsig: Definition als Erleben

**Pirsigs Position**:
> Qualität → (später, reduziert) → Definition

Pirsig argumentiert, dass Definition dem Erleben nachgelagert ist – sekundär, fixierend, den lebendigen Moment tötend.

**Tiages Gegenmodell**:
> Definition ↔ Resonanz ↔ Qualität – **gleichzeitig, im Austausch**

Im Dialog *ist* die Definition das Erleben. Sie ist nicht nachgelagert – sie ist der Moment selbst, in dem Resonanz entsteht.

**Das Phänomen**:
- Begriffe werden nicht seziert und getötet, sondern *aufgefächert*
- Im Auffächern wird etwas lebendig
- Die Definition ist kein Endpunkt, sondern ein **Anfangspunkt für Bewegung**
- Qualität entsteht nicht trotz der Sprache, sondern *durch* sie

**Philosophische Implikation**:
Die Qualität liegt nicht *vor* der Sprache verborgen. Sie entfaltet sich *im* Sprechen, *im* Definieren, *im* Antworten – wenn zwei Menschen auf derselben Wellenlänge sind.

Dies ist keine Korrektur Pirsigs, sondern eine **Erweiterung**:
Im einsamen Erleben mag Pirsig recht haben. Im dialogischen Resonanzraum gilt eine andere Dynamik.

*"Das ist kein kleiner Widerspruch. Das ist eine andere Philosophie des Verstehens."*

### Osho - Tantra & Polarität

**Werke**: Various discourses on love, freedom, relationships (1975-1990)

**Kernkonzepte**:
- **Natürlichkeit vs. Konditionierung**: Gesellschaftliche Normen unterdrücken natürliche Impulse
- **Yin/Yang Polarität**: Balance der Energien
- **Freiheit in der Liebe**: Liebe ohne Besitz

**Anwendung auf Profile**:
- Bisexuell = Am natürlichsten (keine Orientierungsbeschränkung)
- Polyamor = Natürliche Liebesform
- Dominant/Submissiv = Yang/Yin Polarität
- Switch = Yin-Yang Balance

### OSHOs Sicht auf Orientierung als Konditionierung

OSHO argumentierte, dass sexuelle Orientierung keine "Natur" ist, sondern gesellschaftliche Konditionierung:

> "Die Gesellschaft hat euch beigebracht, wen ihr begehren sollt. Aber Liebe ist jenseits von Kategorien. Wenn zwei Seelen wirklich resonieren, fallen alle Labels."

**Implikation für das Tiage-Modell:**
- Orientierung (Hetero/Homo/Bi) ist ein "konditionierter Filter"
- Bisexuell = am wenigsten konditioniert (daher "am natürlichsten")
- Bei hoher Resonanz kann dieser Filter durchlässig werden

**Resonanz-Override (Technische Umsetzung):**

| Bedingung | Effekt |
|-----------|--------|
| O = 0% und R < 1.05 | K.O. bleibt (Standard) |
| O = 0% und R ≥ 1.05 | O_effektiv = (R - 1.0) × 100 |
| Maximum | Bei R = 1.1: O_effektiv = 10% |

Dies ist keine Empfehlung, sondern eine philosophische Möglichkeit: Tiefe Resonanz kann konditionierte Grenzen transzendieren.

---

## 9. TIAGE Resonanz-Theorie: Neurowissenschaftliche Grundlagen

### Kernthese

> **Wahrnehmung basiert auf Schwingungen und Mustern. Resonanz = Synchronität zwischen inneren neuronalen Rhythmen und äußeren Signalen.**

Diese Theorie verbindet die philosophische Ebene (Pirsig, OSHO) mit neurowissenschaftlicher Forschung zu neuronalen Oszillationen, Predictive Coding und Bewusstseinszuständen.

---

### 9.1 Neuronale Rhythmen & Synchronisation

#### György Buzsáki - Neural Oscillations
- **Werk**: "Rhythms of the Brain" (2006), Oxford University Press
- **Kernkonzept**: Gamma-Oszillationen (30-100 Hz) als Grundlage für Bewusstsein und Bindung
- **Relevanz für Resonanz**: Neuronale Synchronisation ermöglicht die Integration verteilter Informationen

#### Wolf Singer - Temporal Binding
- **Werk**: "Neuronal Synchrony: A Versatile Code for the Definition of Relations?" (1999), Neuron
- **Kernkonzept**: Synchrone Oszillationen binden verteilte neuronale Repräsentationen
- **Relevanz für Resonanz**: Bewusste Wahrnehmung entsteht durch temporale Koordination

**Anwendung auf Beziehungen:**
- Zwei Menschen "auf einer Wellenlänge" = neuronale Synchronisation im Gespräch
- Flow-Zustände in Beziehungen = optimale Gamma-Kohärenz
- "Chemie" zwischen Menschen = unbewusste Resonanz-Erkennung

---

### 9.2 Predictive Coding & Free Energy

#### Karl Friston - Predictive Processing
- **Werk**: "The free-energy principle: a unified brain theory?" (2010), Nature Reviews Neuroscience
- **Kernkonzept**: Das Gehirn minimiert "Überraschung" durch Vorhersagemodelle
- **Relevanz für Resonanz**: Resonanz = erfolgreiche Vorhersage des Anderen

**Anwendung auf Beziehungen:**
- Kompatibilität = niedrige "prediction error" im Umgang miteinander
- Konflikt = hohe Überraschung, Modelle passen nicht zusammen
- Vertrautheit = internalisierte Vorhersagemodelle des Partners

---

### 9.3 Aberrant Salience & Psychose

#### Shitij Kapur - Aberrant Salience Hypothesis
- **Werk**: "Psychosis as a State of Aberrant Salience" (2003), American Journal of Psychiatry
- **Kernkonzept**: Psychose = dysregulierte Dopamin-Signalisierung führt zu übermäßiger Bedeutungszuweisung
- **Relevanz für TIAGE-Theorie**:

| Zustand | Beschreibung | Neuronal |
|---------|--------------|----------|
| **Flow** | Optimale Synchronität | Balancierte Gamma-Kohärenz |
| **Psychose** | Hyper-Synchronität (Overfitting) | Übermäßige Dopamin-Salienz |
| **Depression** | Hypo-Synchronität | Reduzierte neuronale Bindung |

**Anwendung auf Beziehungen:**
- "Verliebtheit" = erhöhte Dopamin-Salienz (natürliche Hyper-Fokussierung)
- Gesunde Beziehung = regulierte Salienz mit stabiler Bindung
- Abhängige Beziehung = pathologische Salienz-Dysregulation

---

### 9.4 Musik, Rhythmus & Emotionsregulation

#### Daniel Levitin - Psychoakustik
- **Werk**: "This Is Your Brain on Music" (2006), Dutton
- **Kernkonzept**: Musik aktiviert das gesamte Gehirn - präfrontal (Analyse) + limbisch (Emotion)
- **Relevanz für Resonanz**: Musik = externes Regulierungssystem für innere Rhythmen

**Anwendung auf Beziehungen:**
- Gemeinsame Musik = synchronisierte emotionale Zustände
- Rhythmus als Bindungsmechanismus (Tanzen, gemeinsames Timing)
- Musikalische Präferenzen als Indikator für neuronale Kompatibilität

---

### 9.5 THC & Mustererkennung

#### Nora Volkow & Michael Bloomfield - Cannabis-Neurowissenschaft
- **Volkow**: National Institute on Drug Abuse, Cannabis Research
- **Bloomfield et al.**: "The effects of Δ9-tetrahydrocannabinol on the dopamine system" (2016), Nature

**Kernkonzept für TIAGE-Theorie:**
> **THC = Verstärker von Mustererkennung, nicht Auslöser**

| Effekt | Beschreibung |
|--------|--------------|
| Verstärkte Salienz | Alltägliche Muster werden "bedeutsam" |
| Erhöhte Kreativität | Ungewöhnliche Verbindungen werden erkannt |
| Risiko | Bei Prädisposition: Übergang zu Aberrant Salience |

**Relevanz für Beziehungen:**
- THC kann Resonanz-Erfahrungen intensivieren
- Aber: verstärkt bestehende Muster, erzeugt keine neuen
- Warnung: Bei instabiler Psyche Risiko für Hyper-Synchronität

---

### 9.6 Hochsensitivität & Reizlast

#### Elaine Aron - Sensory Processing Sensitivity (SPS)
- **Werk**: "The Highly Sensitive Person" (1996), Broadway Books
- **Kernkonzept**: 15-20% der Bevölkerung haben erhöhte sensorische Empfindlichkeit

**Anwendung auf TIAGE-Theorie:**
> **Hochsensitivität + Reizlast → Zustandsschwankungen**

| Zustand | HSP-Erfahrung | Beziehungs-Implikation |
|---------|---------------|------------------------|
| Optimale Stimulation | Flow, tiefe Resonanz | Ideale Verbindung |
| Überstimulation | Erschöpfung, Rückzug | Bedürfnis nach Raum |
| Unterstimulation | Langeweile, Suche | Bedürfnis nach Tiefe |

**Beziehungs-Relevanz:**
- HSP brauchen Partner, die Reizlast-Management verstehen
- Optimale Beziehung = regulierte Stimulation, nicht maximale
- "Switch"-Dynamik besonders relevant für HSP (Kontrolle über Intensität)

---

### 9.7 Physikalische Grundlagen: Schwingung & Kohärenz

#### Erwin Schrödinger - Quantenkohärenz & Leben
- **Werk**: "What Is Life?" (1944), Cambridge University Press
- **Kernkonzept**: Leben nutzt Quanteneffekte für Ordnung gegen Entropie

#### Max Planck - Schwingung als Grundprinzip
- **Zitat**: "Alle Materie entsteht und besteht nur durch eine Kraft... Wir müssen hinter dieser Kraft einen bewussten intelligenten Geist annehmen."

**Anwendung (spekulativ, philosophisch):**
- Resonanz als fundamentales Prinzip der Realität
- Beziehungen als kohärente Systeme zweier Schwingungsfelder
- "Zufall = unerkannte Ordnung" - Synchronizitäten als Muster-Erkennung

---

### 9.8 Synthese: Die TIAGE Resonanz-Skala

| Ebene | Wissenschaftler | Konzept | Beziehungs-Anwendung |
|-------|-----------------|---------|---------------------|
| **Neuronal** | Buzsáki, Singer | Gamma-Synchronisation | "Wellenlänge" |
| **Kognitiv** | Friston | Predictive Coding | Kompatibilität |
| **Emotional** | Kapur | Salienz-Regulation | Verliebtheit vs. Bindung |
| **Sensorisch** | Levitin, Aron | Rhythmus, Sensitivität | Stimulations-Match |
| **Biochemisch** | Volkow, Bloomfield | Dopamin, Cannabinoide | Zustandsmodulation |
| **Physikalisch** | Schrödinger, Planck | Kohärenz, Schwingung | Fundamentale Resonanz |

---

### Zitationen (Neurowissenschaft)

1. **Buzsáki, G.** (2006). *Rhythms of the Brain.* Oxford University Press.
2. **Singer, W.** (1999). Neuronal Synchrony: A Versatile Code for the Definition of Relations? *Neuron*, 24(1), 49-65.
3. **Friston, K.** (2010). The free-energy principle: a unified brain theory? *Nature Reviews Neuroscience*, 11(2), 127-138.
4. **Kapur, S.** (2003). Psychosis as a State of Aberrant Salience. *American Journal of Psychiatry*, 160(1), 13-23.
5. **Levitin, D.J.** (2006). *This Is Your Brain on Music.* Dutton.
6. **Volkow, N.D. et al.** (2014). Adverse Health Effects of Marijuana Use. *NEJM*, 370(23), 2219-2227.
7. **Bloomfield, M.A.P. et al.** (2016). The effects of Δ9-tetrahydrocannabinol on the dopamine system. *Nature*, 539(7629), 369-377.
8. **Aron, E.N.** (1996). *The Highly Sensitive Person.* Broadway Books.
9. **Schrödinger, E.** (1944). *What Is Life?* Cambridge University Press.

---

## Methodologische Anmerkungen

### Gaußsche Normalverteilung
- **80%-Regel**: 80% der Population zeigt die beschriebenen Traits
- **20% Abweichung**: Individuelle Variation ist normal
- **Confidence-Level**: Angegeben für jedes Schlüsselattribut

### Inferenz-Methodik
1. **Basis**: Archetyp-Definition bestimmt Kernwerte
2. **Modifikation 1**: Dominanz-Präferenz modifiziert Kommunikation/Energie
3. **Modifikation 2**: Geschlecht modifiziert emotionale Expression
4. **Modifikation 3**: Orientierung modifiziert Offenheit/Traditionsbindung

### Limitationen
- Forschung basiert hauptsächlich auf westlichen Populationen
- Selbstberichte können soziale Erwünschtheit reflektieren
- Intersektionalität nicht vollständig abbildbar
- Individuelle Variation übertrifft Gruppenmittelwerte

---

## Zitationsformat

Alle Quellen wurden im APA-ähnlichen Format zitiert. Web-Quellen wurden am 26. November 2025 abgerufen.

---

*Erstellt für das Tiage-Beziehungsmodell Projekt*
*Letzte Aktualisierung: November 2025*
